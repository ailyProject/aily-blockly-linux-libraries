# CircuitPython BNO055 Blockly 库

面向树莓派和 Linux 单板机的 CircuitPython BNO055 常用 API 白名单积木。

- 目标端安装：`python3 -m pip install adafruit-circuitpython-bno055`
- 积木（3 个）：`python_adafruit_bno055_call`、`python_adafruit_bno055_do`、`python_adafruit_bno055_attribute`
- 可调用入口：`BNO055_I2C`、`BNO055_UART`
- 对象方法：无
- 对象/模块属性：`sensor_mode`、`temperature`、`acceleration`、`magnetic`、`gyro`、`euler`、`quaternion`、`linear_acceleration`、`gravity`、`calibration_status`、`calibrated`
- API 文档：https://docs.circuitpython.org/projects/bno055/en/latest/
...
支持级别：有条件。BNO055 存在 I2C clock stretching；必要时将 I2C 波特率降至 10 kHz，或改用 UART。
