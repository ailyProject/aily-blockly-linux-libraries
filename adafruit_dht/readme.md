# CircuitPython DHT Blockly 库

面向树莓派和 Linux 单板机的 CircuitPython DHT 常用 API 白名单积木。

- 目标端安装：`python3 -m pip install adafruit-circuitpython-dht`
- 积木（5 个）：`python_adafruit_dht_call`、`python_adafruit_dht_do`、`python_adafruit_dht_method`、`python_adafruit_dht_do_method`、`python_adafruit_dht_attribute`
- 可调用入口：`DHT11`、`DHT22`
- 对象方法：`exit`
- 对象/模块属性：`temperature`、`humidity`
- API 文档：https://docs.circuitpython.org/projects/dht/en/latest/
...
支持级别：实验性。Adafruit 明确提示 Linux 64 位 DHT 读取可能不可靠；必要时使用 `use_pulseio=False`、捕获 `RuntimeError` 并重试。新设计优先选 AHTx0、BME280 或 SCD4x。
