/* Curated CircuitPython PN532 bridge for Raspberry Pi and Linux CPython projects. */
(function (root, spec) {
  'use strict';
  ...
  const callableTargets = Object.freeze({
    PN532_I2C: Object.freeze({
      importKey: spec.importKey,
      module: spec.module,
      alias: spec.alias,
    }),
    PN532_SPI: Object.freeze({
      importKey: 'python_lib_adafruit_pn532_spi',
      module: 'adafruit_pn532.spi',
      alias: '_python_lib_adafruit_pn532_spi',
    }),
  });
  if ([...callables].some((name) => callableTargets[name] == null)) {
    throw new Error(`${spec.title} has a callable without a fixed import target`);
  }
  const callableTarget = (requested) => {
    const name = selected(callables, requested);
    const target = callableTargets[name] ?? callableTargets.PN532_I2C;
    return { ...target, name };
  };
  const addCallableImport = (generator, target) =>
    generator.addImport(target.importKey, `import ${target.module} as ${target.alias}`);
  ...
  const callCode = (generator, target, args, kwargs, importTarget = null) => {
    if (importTarget == null) addImport(generator);
    else addCallableImport(generator, importTarget);
  ...
  Python.forBlock[`${spec.prefix}_call`] = (block, generator) => {
    const target = callableTarget(field(block, 'TARGET'));
    const code = callCode(
      generator,
      dotted(target.alias, target.name),
  ...
  if (attributes.size) {
    ...
    "title": "CircuitPython PN532",
    "prefix": "python_adafruit_pn532",
    "module": "adafruit_pn532.i2c",
    "alias": "_python_lib_adafruit_pn532",
    "importKey": "python_lib_adafruit_pn532",
    "callables": [
      "PN532_I2C",
      "PN532_SPI"
    ],
