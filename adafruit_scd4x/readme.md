# CircuitPython SCD4x Blockly 库

面向树莓派和 Linux 单板机的 CircuitPython SCD4x 常用 API 白名单积木。

- 目标端安装：`python3 -m pip install adafruit-circuitpython-scd4x`
- 积木（5 个）：`python_adafruit_scd4x_call`、`python_adafruit_scd4x_do`、`python_adafruit_scd4x_method`、`python_adafruit_scd4x_do_method`、`python_adafruit_scd4x_attribute`
- 可调用入口：`SCD4X`
- 对象方法：`start_periodic_measurement`、`stop_periodic_measurement`、`reinit`、`persist_settings`、`force_calibration`
- 对象/模块属性：`CO2`、`temperature`、`relative_humidity`、`data_ready`、`measurement_period`、`self_calibration_enabled`
- API 文档：https://docs.circuitpython.org/projects/scd4x/en/latest/
