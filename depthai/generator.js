/* Curated DepthAI v3 bridge for Raspberry Pi and Linux CPython projects. */
(function (root, spec) {
  'use strict';
  ...
  const nodeAttributes = new Set(spec.attributes);
  ...
  if (attributes.size) {
    Python.forBlock[`${spec.prefix}_attribute`] = (block, generator) => {
      ...
      if (nodeAttributes.has(name)) return output(`${spec.alias}.${name}`, ORDER_MEMBER);
      const object = value(generator, block, 'OBJECT', spec.alias);
    ...
    "title": "DepthAI v3",
    "prefix": "python_depthai",
    "module": "depthai",
    "alias": "_python_lib_depthai",
    "importKey": "python_lib_depthai",
    "callables": [
      "Pipeline",
      "Device"
    ],
