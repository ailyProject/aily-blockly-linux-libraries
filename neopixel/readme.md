# CircuitPython NeoPixel Blockly 库

面向树莓派和 Linux 单板机的 CircuitPython NeoPixel 常用 API 白名单积木。

- 目标端安装：`python3 -m pip install adafruit-circuitpython-neopixel`
- 积木（5 个）：`python_neopixel_call`、`python_neopixel_do`、`python_neopixel_method`、`python_neopixel_do_method`、`python_neopixel_attribute`
- 可调用入口：`NeoPixel`
- 对象方法：`fill`、`show`、`deinit`
- 对象/模块属性：`brightness`、`auto_write`、`n`、`bpp`
- API 文档：https://docs.circuitpython.org/projects/neopixel/en/latest/
...
兼容性说明：树莓派 5 需要受支持的 SPI 或 PIO 后端以及相应设备权限。
...
支持级别：有条件。Pi 5 需要当前 PIO 后端和 `/dev/pio0` 权限，或使用会占用 SPI 总线的 SPI 回退方案；不要沿用旧 RPi.GPIO/rpi_ws281x 直写路径。
