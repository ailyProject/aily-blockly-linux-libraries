/* Curated watchdog Observer bridge for Raspberry Pi and Linux CPython projects. */
(function (root, spec) {
  'use strict';
  ...
  const addImports = (generator) => {
    generator.addImport(
      spec.importKey,
      `import ${spec.module} as ${spec.alias}`,
    );
    generator.addImport(
      `${spec.importKey}_events`,
      `import watchdog.events as ${spec.alias}_events`,
    );
  };
  const callableTargets = Object.freeze({
    Observer: `${spec.alias}.Observer`,
    LoggingEventHandler: `${spec.alias}_events.LoggingEventHandler`,
    FileSystemEventHandler: `${spec.alias}_events.FileSystemEventHandler`,
    PatternMatchingEventHandler: `${spec.alias}_events.PatternMatchingEventHandler`,
  });
  const callableTarget = (name) => callableTargets[name] ?? callableTargets.Observer;
  ...
  if (attributes.size) {
    ...
    "title": "watchdog Observer",
    "prefix": "linux_watchdog",
    "module": "watchdog.observers",
    "alias": "_python_lib_watchdog",
    "importKey": "python_lib_watchdog",
    "callables": [
      "Observer",
      "LoggingEventHandler",
